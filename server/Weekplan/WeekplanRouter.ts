...
export default ({ controller, jsonResult, auth }: { controller: WeekplanController, jsonResult: JSONHandler, auth: Auth }): Router => {
  ...
  function getWeekplan(req: Request) {
    return controller.getWeekplan(
      req.user as User,
      req.params.date,
      req.query && req.query.inhibit ? (req.query.inhibit as string).split(',') : []
    )
  }

  function fixPlan(req: Request) {
    return controller.fixPlan(req.user as User, req.params.date, req.body.accepted)
  }
