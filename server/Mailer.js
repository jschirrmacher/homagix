...
const baseUrl = process.env.BASEURL || 'http://localhost:' + (process.env.PORT || 8200)
const from = process.env.MAIL_FROM || 'me@localhost'

export default ({ nodemailer }) => {
  async function send(to, templateName, variables) {
    const template = (await import('./mailTemplates/' + templateName + '.js')).default
    return new Promise((resolve, reject) => {
      const subject = Mustache.render(template.subject, { baseUrl, ...variables })
