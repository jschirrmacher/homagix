...
describe('DishesRouter', () => {
  ...
  describe('POST /dishes', () => {
    ...
    it('should add a dish', async () => {
      ...
      saved.items && saved.items.map(i => ({ ...models.ingredient.byId(i.id), amount: i.amount })).should.containDeep(items)
    })
  ...
  describe('PATCH /dishes/:id', () => {
    ...
    it('should update the recipe of a dish', async () => {
      ...
      should(models.dish.byId('4711').recipe).equal('Place the patty in the bun')
    })
