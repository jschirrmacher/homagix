...
export default function ({
  ...
  const { dishAdded, dishModified, itemRemoved, itemAmountUpdated } = models.dish.events
  ...
  return {
    ...
    async updateDish(
      ...
      await store.emit(dishModified(JSON.parse(JSON.stringify({ id, name, recipe, source }))))
    ...
    async addItem(dishId: string, item: ReadableItem, user: User): Promise<Dish> {
    ...
    async removeItem(
      ...
      await store.emit(itemRemoved(dishId, itemId))
      return getSingleDish(dishId, user)
    },
    ...
    async updateItemAmount(
      ...
      await store.emit(itemAmountUpdated(dishId, itemId, amount))
      return getSingleDish(dishId, user)
    },
  }
}
