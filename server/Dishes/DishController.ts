...
export default function ({ store, models, dishReader }: { store: Store, models: Models, dishReader: DishReader }): DishController {
  ...
  return {
    ...
    async updateDish(id: string, dishPartial: Partial<Dish>, user: User): Promise<Dish> {
      await store.emit(dishModified({ ...dishPartial, id }))
      return getSingleDish(id, user)
    },

    async setFavorite(user: User, dishId: string, isFavorite: boolean): Promise<Dish> {
